package telegram

import (
	"testing"

	"github.com/stretchr/testify/require"

	"github.com/gotd/td/exchange"
	"github.com/gotd/td/tg"
)

func Test_parseCDNKeys(t *testing.T) {
	keys := []string{
		`-----BEGIN RSA PUBLIC KEY-----
MIIBCgKCAQEA+Lf3PvgE1yxbJUCMaEAk

mQIDAQAB
-----END RSA PUBLIC KEY-----`,
		`-----BEGIN RSA PUBLIC KEY-----

FRX7DdwIA/FdOzfWyXYLlCFaSX8K/6CnSQIDAQAB
-----END RSA PUBLIC KEY-----`,
	}

	cdnKeys := make([]tg.CDNPublicKey, 0, len(keys))
	for i, key := range keys {
		cdnKeys = append(cdnKeys, tg.CDNPublicKey{
			DCID:      i + 1,
			PublicKey: key,
		})
	}

	publicKeys, err := parseCDNKeys(cdnKeys...)
	require.NoError(t, err)
	require.Len(t, publicKeys, 2)
}

func Test_mergePublicKeys(t *testing.T) {
	keys := []string{
		`-----BEGIN RSA PUBLIC KEY-----
MIIBCgKCAQEA+Lf3PvgE1yxbJUCMaEAk

mQIDAQAB
-----END RSA PUBLIC KEY-----`,
		`-----BEGIN RSA PUBLIC KEY-----

FRX7DdwIA/FdOzfWyXYLlCFaSX8K/6CnSQIDAQAB
-----END RSA PUBLIC KEY-----`,
	}

	parsed, err := parseCDNKeys(
		tg.CDNPublicKey{DCID: 1, PublicKey: keys[0]},
		tg.CDNPublicKey{DCID: 2, PublicKey: keys[1]},
	)
	require.NoError(t, err)
	require.Len(t, parsed, 2)

	base := []exchange.PublicKey{{RSA: parsed[0]}}
	extra := []exchange.PublicKey{{RSA: parsed[0]}, {RSA: parsed[1]}}

	merged := mergePublicKeys(base, extra)
	require.Len(t, merged, 2)
}
